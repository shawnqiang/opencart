...
$('#button-voucher').bind('click', function() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#button-voucher').after('<span class="wait">&nbsp;<img src="catalog/view/theme/uhcenter/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
		...
		success: function(json) {
			if (json['error']) {
				$('#basket').before('<div class="warning">' + json['error'] + '<img src="catalog/view/theme/uhcenter/image/close.png" alt="" class="close" /></div>');
			}
