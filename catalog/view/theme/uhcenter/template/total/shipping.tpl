...
$('#button-quote').bind('click', function() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#button-quote').after('<span class="wait">&nbsp;<img src="catalog/view/theme/uhcenter/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
...
$('#button-shipping').live('click', function() {
	$.ajax({
		...
		success: function(json) {
			if (json['error']) {
				$('#shipping').before('<div class="warning">' + json['error'] + '<img src="catalog/view/theme/uhcenter/image/close.png" alt="" class="close" /></div>');
			}
